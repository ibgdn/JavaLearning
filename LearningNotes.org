...
*** Oracle
    ...
    1. 字符串连接
       ...
       > select first_name ||' '|| last_name 姓名 from hr.employees;
    2. 去除重复
       查询部分 id
       > select distinct department_id from hr.employees;
    3. 条件限定
       名字里倒数第2个字母是 a 的员工（%任意匹配，有货没有；_有并且只有一个）
    ...
    6. 子查询
       子查询即可以简单的理解成同时执行多条sql语句，将一条sql语句的查询结果作为条件来执行另一条sql语句。
    ...
    7. 分页查询
       ...
       >  (select e.* from hr.employees e order by e.salary desc) e1
       > ) e2 where e2.r >= 5 and e2.r <= 10;
    8. 序列 sequence
       每次插入数据的时候，id应该自动增加的，那么就需要使用sequence作为id自增长技术 。
       创建 sequence,序列从1开始，每次增加1，最大值为9999999。
       > create sequence seq
       >   increment by 1
       >   start with 1
       >   maxvalue 9999999
       ...
       > values (seq.nextval, '炸弹人')；
    9. 修改数据
       > update hero set name = '修补匠' where id = 22;
    10. 舍弃表
        truncate 立即删除表里所有的记录，比 delete 更快，更彻底，并且不能回复，不能回滚！！！
        > truncate table hero;
    11. 修改表结构
        新增列
        > alter table hero add (kills number);

        修改列（将 name 原有字段类型修改为300长度的变长字符）
    ...
    13. 约束
        ...
        >   id number, killerid number, killedid number, number_ number,
